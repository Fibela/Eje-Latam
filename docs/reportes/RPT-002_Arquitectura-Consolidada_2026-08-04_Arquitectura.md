...
| PA-146 | **No existe procedimiento de rotación de identidad del sensor.** Cambiar `clave-cliente.pub` deja cualquier `inventario.inv` anterior sin verificar, y `arranque.rs` lo lee como **manipulación**; borrarlo dejando el centinela establecido se lee como **supresión**. Las dos lecturas son correctas: el almacén no distingue una rotación legítima de un ataque porque **nadie se lo ha dicho nunca** | 🔴 **Abierto, crítico** — 31-ago-2026. Hoy la operación son cuatro `rm` a mano de un operador, indistinguibles de la supresión que el almacén está diseñado para detectar. Es justo la maniobra que hay que hacer con prisa y sin equivocarse ante una semilla comprometida, y no tiene ni orden en `eje-manifiesto` ni acta. Salió de perder la semilla de la VM de PA-78 |
