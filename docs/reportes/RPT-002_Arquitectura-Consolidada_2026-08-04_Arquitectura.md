...
| PA-138b | **Componer `NodoInventario` y cablear el canal.** Ya hay donde leer (PA-138a) y ya hay que decir (PA-139) | 🟡 Abierto — RPT-088 §6. **Desbloqueado.** Falta la traduccion de `VistaNodo` mas marcado firmado a `ClaseConocida`, el mapeo de `DeclaracionSegmento` al cable —como `perfil_en_el_cable`—, el manejador, y `servido = true` |
