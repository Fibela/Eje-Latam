...
| PA-138b | **Componer `NodoInventario` y cablear el canal.** | 🔵 **Parcial** 28-ago-2026 — RPT-090. **Cerrado por construccion:** tercer canal de los seis servido, con `clase_en_el_cable` como `match` de once brazos **sin comodin** — que en su primer dia cazo un quinto `MotivoAmbiguedad` que no habiamos visto, y precisamente el que significa manipulacion del inventario. El inventario sale del volatil entero y no de lo visto en la vuelta, y una fuente declarativa que falla sale como `ambiguaEvidenciaNoVerificable` y no como ausencia de marcado. **Falta la observacion en la VM**: nadie lo ha visto responder en maquina real |
