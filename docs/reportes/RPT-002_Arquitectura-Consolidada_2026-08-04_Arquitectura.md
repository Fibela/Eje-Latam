...
| PA-139 | **`Postura` no tenia valor para «no se sabe», mezclaba juicio con estado operativo — y ademas no tenia productor.** Su unica aparicion en Rust era un dato de prueba; en TypeScript si tenia consumidor | ✅ **Cerrado** 28-ago-2026 — RPT-088. No se le anadio una cuarta variante: **se retiro del contrato**. El agente sabe `Indicio`, `DeclaracionSegmento` y la marca de segmento critico; eso no es una postura, son las evidencias con las que se forma una. `NodoInventario` pasa a llevar evidencia y el juicio lo compone VIS-04, donde cambiar la regla no exige recompilar el sensor. Tambien se fue `identificador`, que era la misma cadena que `direccionEnlace`. La clase lleva su **procedencia dentro** (`declaradaSoporteVital` frente a `inferidaSoporteVital`): con dos campos alguien leeria la clase sin mirar el respaldo, con uno la lectura equivocada no se puede escribir |
