...
| ~~PA-78~~ | ~~**Nadie ha visto a los dos procesos hablarse.** Los vectores prueban el formato, no la conversación~~ | ✅ **Cerrado por observación** 06-sep-2026 — RPT-093. **Mitad B cerrada contra una pantalla real**, con captura en `docs/evidencia/RPT-093_VIS-04_2026-09-06.png`. VIS-04 sobre XFCE en `eje-prueba`, contra el socket por omisión del contrato (`/run/eje-latam/agente.sock`, sin `EJE_SOCKET`), con el agente como servicio y el usuario en el grupo `eje-ipc`. Las trece condiciones, el respaldo y el inventario, con el dato que el sensor observa: `00:00:00:00:00:00 · ambiguaSegmentoPuedeAlojarCriticos · segmento noDeclarado · visto en segmento crítico`. **La predicción escrita antes acertó carácter a carácter en la fila del inventario y en los cinco contadores del respaldo**, y falló en la cabecera — de ahí PA-148. Lo que costó no fue el código: fue que el binario desplegado llevaba desde el 25 de agosto sin actualizarse (§ RPT-093 §4) |
...
| ~~PA-138~~ | ~~**`obtener-inventario` no tiene productor posible.**~~ | ✅ **Partido** 28-ago-2026 — RPT-087 §2. Eran dos trabajos con un numero, y ademas cada mitad esperaba a PA-139 por un motivo distinto: enumerar no toca el contrato, componer la respuesta si |
| PA-138a | **`AlmacenObservacion` sabia contar y no sabia enumerar.** `volatiles()` y `pegajosos()` devuelven `usize`; un canal que promete una lista no se puede escribir contra eso | ✅ **Cerrado** 28-ago-2026 — RPT-087. `inventario()` enumera el volatil ordenado por direccion, con protocolos observados, declaracion de segmento, reloj y marca pegajosa. **Sin `clase` ni `postura`**: son justo lo que PA-139 tiene que decidir, y rellenarlos aqui seria inventar dato. Aparecio un tercer estado que no estaba en la fila: un pegajoso ya expulsado del volatil no se afirma presente ni se pierde — sale por `pegajosos_no_observados()` |
| PA-138b | **Componer `NodoInventario` y cablear el canal.** | ✅ **Cerrado por observacion** 28-ago-2026 — RPT-090. tercer canal de los seis servido, con `clase_en_el_cable` como `match` de once brazos **sin comodin** — que en su primer dia cazo un quinto `MotivoAmbiguedad` que no habiamos visto, y precisamente el que significa manipulacion del inventario. El inventario sale del volatil entero y no de lo visto en la vuelta, y una fuente declarativa que falla sale como `ambiguaEvidenciaNoVerificable` y no como ausencia de marcado. **Observado en la VM** (md5 comprobado en los dos lados): responde en 393 ms con `ambiguaSegmentoPuedeAlojarCriticos` — ninguna `declarada*`, porque esa MAC no esta en ningun marcado firmado. El agente no se inventa una clasificacion para un equipo del que no sabe nada |
| PA-139 | **`Postura` no tenia valor para «no se sabe», mezclaba juicio con estado operativo — y ademas no tenia productor.** Su unica aparicion en Rust era un dato de prueba; en TypeScript si tenia consumidor | ✅ **Cerrado** 28-ago-2026 — RPT-088, **corregido por RPT-089 el mismo dia**. No se le anadio una cuarta variante: **se retiro del contrato**. El agente sabe `Indicio`, `DeclaracionSegmento` y la marca de segmento critico; eso no es una postura, son las evidencias con las que se forma una. `NodoInventario` pasa a llevar evidencia y el juicio lo compone VIS-04, donde cambiar la regla no exige recompilar el sensor. Tambien se fue `identificador`, que era la misma cadena que `direccionEnlace`. La clase lleva su **procedencia dentro** en un solo valor: con dos campos alguien leeria la clase sin mirar el respaldo, con uno la lectura equivocada no se puede escribir. **RPT-089:** la primera version del enumerado afirmaba clases inferidas que `clasificar` nunca devuelve —una fuente inferida no afirma, levanta la mano— y le faltaba `declaradaNoCritica`, el unico estado que permite accion automatica. Ahora son diez valores, espejo uno a uno de los resultados alcanzables de `clasificar`. No lo cazo ninguna prueba: la paridad comprueba que los tres lados digan lo MISMO, no que digan la VERDAD |
...
| PA-136 | **El plano de control esperaba al de datos, y la ventana no tenia techo.** La latencia de la consola **era** la ventana de captura, y el bucle no estaba acotado por `PLAZO`: seguia mientras llegaran tramas | ✅ **Cerrado por observacion** 28-ago-2026 — RPT-084. `atender` se llama ahora **entre trama y trama**, sirviendo el estado ya persistido de la vuelta anterior (RPT-034 §4 intacto, misma edad de dato). En la primera vuelta las condiciones **se rechazan con motivo** en lugar de devolver trece `false` —RPT-006 §4 en un sitio nuevo— y el rechazo alcanza solo a ese canal: las alertas en disco se sirven igual. En la VM, mismo goteo que ayer: **seis de seis responden, cero vencimientos** (antes tres a 5000 ms). Lo que cambio no es el numero sino que ahora hay techo: `latencia ≤ min(intervalo entre tramas, PLAZO)`. Salieron PA-140 y PA-141 |
| PA-140 | **La latencia de consulta queda atada a `PLAZO`.** Tras RPT-084 la espera es `min(intervalo entre tramas, PLAZO)`; si alguien sube `PLAZO` la latencia sube con el y ninguna prueba lo notaria | 🟡 Abierto — RPT-084 §8. Observado a **499 ms**, a un milisegundo del plazo |
| PA-141 | **El rechazo de la primera vuelta no se habia observado en maquina real.** Estaba cubierto por dos unitarias, pero nadie habia visto a la consola recibirlo | ✅ **Cerrado por observacion** 28-ago-2026 — RPT-084 §4.1. Con el guion lanzado a los 2 s, `obtener-condiciones` devolvio `RECHAZO` con el motivo literal, y `consultar-alertas` y `obtener-estado-agente` sirvieron igual: el rechazo alcanza solo al canal que carece del dato |
| PA-142 | **Los ficheros del renderer son ciegos.** No los compila `tsc`, no los cruza `dependency-cruiser` y ninguna prueba los ejecuta | 🔵 **Parcial** 28-ago-2026 — RPT-091. **`vis04` ya no lo es:** vive en `vista/src/vis04.ts`, se emite a `vista/dist/` (no versionado) y `window.eje` esta tipado como `PuenteEje`, que la paridad ata al contrato y el contrato al struct de Rust. **Un campo que el agente no manda ya no compila la vista.** La tabla de condiciones lleva `satisfies keyof Condiciones`, asi que media barrera textual la hace ahora el compilador. Falta `diagnostico.js`, que ademas lleva escrito que es JS **a proposito** — con una premisa falsa: si se despliega |
| PA-143 | **La eleccion de Electron no tiene acta.** «tauri» no aparece en todo el repositorio y ningun reporte explica por que Electron y no otra cosa. Un producto que se vende por ser auditable empaqueta un Chromium entero en el artefacto que el cliente instala | 🟡 Abierto — 28-ago-2026. **Resuelto para v1 y decidido para v2:** v1 sale con Electron para no descarrilar el calendario; la directiva para v2 es migrar a Tauri, que cierra el circulo en Rust, erradica el problema cronico del `chrome-sandbox` y reduce superficie de ataque y tamano. Queda abierto hasta que exista el acta escrita, no la intencion |
| PA-144 | **`eje-manifiesto` pedía la frase de paso antes de comprobar que sus entradas existían.** `emitir` y `configurar` imprimían el aviso de PA-53, esperaban la frase en claro sobre la pantalla, y **sólo después** fallaban por un fichero inexistente. `generar` ya lo hacía al revés: el mismo binario llevaba los dos órdenes | ✅ **Cerrado** 31-ago-2026 — RPT-092. No fue un hallazgo de auditoría: **quemó una frase de paso real** aprovisionando la VM de PA-78, que hubo que dar por comprometida. Sale `entradas_sin_secreto`, que lee y analiza todo lo que puede fallar sin conocer secreto alguno — es una función y no dos líneas movidas para que el cuarto comando que abra una semilla se encuentre el orden ya escrito. La barrera vive en **xtask**, no en el crate: lo que comprueba es el orden de dos sentencias, que el compilador no ve, y el único analizador de comentarios del proyecto está en `exclusion.rs`; un segundo en el otro crate sería la duplicación contra la que avisa el propio `Recorrido`. Acota por función, salta el bloque de pruebas y **falla en voz alta si cualquiera de las dos agujas desaparece** en lugar de quedarse verde sin comprobar nada |
| PA-145 | **`--anterior` inexistente se colapsa con `--anterior` ausente.** `Some(ruta) if ruta.exists() => …, _ => None`: una errata en la ruta **rebobina la serie a 1 en silencio**, en `emitir` y en `configurar` | 🟡 Abierto — 31-ago-2026, visto al escribir PA-144. Es RPT-006 §4 en un sitio nuevo: «no se pidió anterior» y «se pidió y no está» son dos hechos distintos y aquí dicen lo mismo. No lo tapa nada aguas abajo salvo el centinela del sensor, que sólo protege si ya aceptó una secuencia mayor — en un aprovisionamiento nuevo no protege nada. Se separa del arreglo de PA-144 a propósito: una cosa por reporte |
| PA-146a | **La ausencia de clave de recuperación era silenciosa.** `arrancar_con_almacen` la cargaba, la usaba para las revocaciones, y el hecho de que faltara **moría en esa función**: `eje-manifiesto generar` lo avisaba por pantalla en la máquina del administrador, y quien instala el sensor no es quien emitió la semilla | ✅ **Cerrado por observación** 08-sep-2026 — RPT-095, captura en `docs/evidencia/`. `Arranque` deja de ser una tupla y saca el tercer hecho que ya tenía en la mano; `sinClaveDeRecuperacion` es la condición **catorce**, entra en `Ciclo` por el **constructor** y no por un `declarar_*` —con valor de partida `false`, olvidarlo significaría «sí hay clave», el supuesto optimista de siempre; por el constructor no compila—, y sale por syslog: es la única condición que el operador del sensor no puede resolver, así que decírsela sólo a él es decírsela a quien no puede hacer nada. Añadir el campo obligó a tocar catorce sitios y **los catorce los exigió el compilador o una prueba**; once comentarios que decían «las trece» dejaron de ser ciertos y ésos no los caza nadie |
| PA-150 | **La memoria de identidad del centinela.** Sin ella, borrar `clave-recuperacion.pub` devolvía al sensor al estado de recién instalado y el operador no podía distinguir una ceremonia pendiente de una credencial suprimida | ✅ **Cerrado por observación** 10-sep-2026 — RPT-096, **tres** capturas en `docs/evidencia/`. Centinela **v3**: las dos marcas más la huella de la clave, con bandera de presencia y bajo su propio dominio del `Absorbedor` (RPT-011 §2). La condición catorce se parte en **tres** —`recuperacionNoAprovisionada`, `recuperacionSuprimida`, `recuperacionNoVerifica`— y las dieciséis se observaron en máquina real cambiando entre sano, saboteado y secuestrado en tres minutos. **El agente no ancla solo:** una clave presente sin anclar se lee como no aprovisionada, o el ataque sería «borro los dos ficheros y dejo la mía». `vacio()` pasa a mirar tres campos, porque un sensor recién aprovisionado era irrepresentable. Y la migración **no es en caliente**: las dos combinaciones cruzadas dejan el sensor caído |
| PA-146b | **El certificado de rotación de identidad.** Cambiar `clave-cliente.pub` deja cualquier `inventario.inv` anterior sin verificar y `arranque.rs` lo lee como **manipulación**; borrarlo dejando el centinela establecido se lee como **supresión**. Las dos lecturas son correctas: el almacén no distingue una rotación legítima de un ataque porque nadie se lo ha dicho nunca | 🔵 **Parcial** 12-sep-2026 — RPT-098. **No hay certificado que diseñar: existe desde RPT-015** y `eje-manifiesto revocar --sucesora` ya lo emite con las cuatro piezas —huella revocada, corte, sucesora e instante— firmadas por la clave de recuperación. Lo que faltaba es la **sucesión**: `ArchivoRevocaciones::registro` tiraba `sucesora` al derivar el registro, y fuera de una prueba no la consumía nadie. **Fase 1 hecha:** el registro la conserva y `sucesora_de` la devuelve, con dos estados y no `Option` porque «no revocada» dice algo que `None` no dice. Y una distinción que el punto necesitaba: el certificado rota la **operativa** y lo firma la de **recuperación**, así que PA-150 dio testigo a la clave menos usada y **la operativa sigue sin ninguno** — se carga del disco y se cree. Faltan el centinela **v4** con el ancla de la operativa y la máquina de cinco estados, donde el quinto es `Rotada`: la huella cambió y hay certificado que lo autoriza |
| ~~PA-146~~ | ~~**No existe procedimiento de rotación de identidad del sensor.**~~ | ✅ **Partido** 08-sep-2026 — RPT-095 §1. Eran dos trabajos con un número, y el segundo dependía del primero por una razón que sólo se vio al mirar el almacén de la VM: la clave con la que se firmaría el certificado **no existía en el sensor desplegado**, y nada lo decía |
| ~~PA-149~~ | ~~**Qué condiciones son «manipulación» está escrito a mano en dos idiomas y nada las compara.**~~ | ✅ **Cerrado** 10-sep-2026 — RPT-097. El hecho pasa a `contrato-ipc.toml` como columna `manipulacion`, declarada en **las dieciséis** y también cuando es `false`: si la ausencia valiera por `false`, añadir una condición y olvidarla la dejaría fuera en silencio, que es el defecto mismo. `hay_manipulacion()` deja de enumerar y cruza `CAMPOS_CONDICIONES` con `enumerar()`. **La cascada de `componerCabecera` NO se deriva** —su orden es la prioridad y su prosa es lo que le dice al operador qué hacer; dieciséis frases generadas serían peores que ninguna cabecera— y en su lugar se sujeta con una prueba que lee el contrato y exige que cada manipulación ocupe cabecera. **Comprobada al revés:** desactivando la rama de `recuperacionNoVerifica`, la barrera se pone roja nombrando la condición. Y el objeto «todo en calma» de esa prueba se **deriva**, porque escribirlo a mano habría sido una lista de dieciséis dentro del arreglo del problema de las listas de dieciséis |
| PA-148 | **La cabecera de VIS-04 no dice qué sensor se está mirando.** Muestra `Eje-Agente 0.1.0 · perfil corporativo · contiene sin aprobación`, y ni `nombre` ni `maquina_esperada` aparecen en ninguna parte de la pantalla | 🟡 Abierto — 06-sep-2026, visto en la captura que cierra PA-78. Los dos campos viajan firmados en la configuración y `nombre` existe **precisamente** para correlacionar sellos y detectar ausencia de latidos (RPT-058); dos sensores con el mismo nombre hacen que el latido de uno tape la muerte del otro. Con una sola máquina no molesta, y ése es el problema: **se nota el día que hay tres consolas abiertas y ninguna dice cuál es cuál**. No lo cazó ninguna prueba porque no hay ninguna que afirme qué debe contener la cabecera, sólo que se compone |
| PA-147 | **La plantilla del instalador contradice al código y a sí misma.** `configuracion_ejemplo()` declara `intervalo_latido_ms = 30000`; `INTERVALO_LATIDO_MS` en `salida.rs` vale **60000**. Dos latidos por omisión para el mismo producto, factor dos, uno en texto y otro en código — nadie los ve juntos. Y `grupo_ipc = 0` justo debajo de un comentario que dice «omítelo para dejar el socket en 0600»: la plantilla hace lo contrario de lo que su propia línea de arriba manda hacer, y quien la copie sin editar se lleva un socket de grupo `root` en 0660 en vez de un 0600 | 🟡 Abierto — 01-sep-2026. Salió al reconstruir el TOML perdido de la VM de PA-78: la plantilla apareció en `/etc/eje-latam/` y desmintió la reconstrucción, que había elegido 60000 por ser el valor del código. **Ninguna barrera podía cazarlo:** `la_plantilla_la_acepta_el_emisor_de_verdad` comprueba que el TOML *analice*, no que diga lo mismo que el binario; es paridad de forma sin paridad de valor, la misma forma que RPT-089 §1 |
...
| PA-97 | `componerSucesos` no leia `hayMas` **y llevaba semanas leyendolo**. La fila estaba caduca: RPT-050 cableo el bucle y nadie la actualizo | ✅ **Cerrado por re-verificacion** 28-ago-2026 — RPT-085. `vis04.js` es la vista por omision, pide `consultarAlertas(bitacora.desdeAsiento)`, incorpora y programa el siguiente sondeo con `esperaSugerida`, que devuelve 0 ms con cola pendiente. No se cierra por construir nada, sino por comprobarlo — y por levantar la barrera que faltaba: dos pruebas nuevas sujetan que la vista **llame** al mecanismo y **en el orden correcto**. Salio PA-142 |
...
| PA-121 | **`cargo xtask conformidad` estaba disenada y no construida.** RPT-005 §9.3 la describia entera y RPT-066 la encontro mandando teclear una orden inexistente | ✅ **Cerrada** 28-ago-2026 — RPT-086. Ejecuta las tres suites poscuanticas y **solo si pasan** emite `CONFORMIDAD.lock`: versiones exactas de las dependencias de `motor-pqc`, resumen de `FUENTES.lock`, canal del toolchain y huella SHA-256 sobre todo ello. La huella se calcula sobre las **entradas**, no sobre el evento de compilacion, asi que subir una dependencia sin reejecutar pone rojo `cargo test -p xtask` solo. El conjunto de paquetes **se deriva del manifiesto** y no se escribe a mano. Ata QUE se probo, no QUE se probo: eso sigue siendo PA-14 |
...
| ~~PA-53~~ | ~~Lectura de la frase de paso sin eco en pantalla~~ | ✅ **Cerrado** 06-sep-2026 — RPT-094. `eje-manifiesto::eco` apaga `ECHO` con `termios` y lo devuelve en `Drop`, **no en una llamada al final**: una salida por error dejaría la terminal muda, que es peor incidente que el eco. `libc` ya estaba en el árbol vía `eje-captura` y resuelta a la misma versión, así que la corrección **no añade un solo paquete**. Y no son dos estados sino **tres**: «apagado», «no es terminal» y «es terminal y no se pudo» — colapsar el tercero en el primero haría que la herramienta dijera «no se verá» sobre una pantalla donde se ve, que es peor que el defecto original, porque hoy quien teclea al menos lo sabe. Segundo módulo del workspace con `unsafe`, con su lista de tres llamadas enumerada; la cabecera de `eje-captura::linux`, que decía ser el único, se corrigió en el mismo cambio. *Histórico:* abierto en RPT-026 §5, escalado a crítico el 31-ago-2026 tras quemar una frase real |
