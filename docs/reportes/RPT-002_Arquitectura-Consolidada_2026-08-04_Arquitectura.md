...
| PA-146b | **El certificado de rotación de identidad.** Cambiar `clave-cliente.pub` deja cualquier `inventario.inv` anterior sin verificar y `arranque.rs` lo lee como **manipulación**; borrarlo dejando el centinela establecido se lee como **supresión**. Las dos lecturas son correctas: el almacén no distingue una rotación legítima de un ataque porque nadie se lo ha dicho nunca | 🔴 **Abierto, crítico** — 08-sep-2026. Firmado por la clave de recuperación, nombrando la huella vieja y la nueva. **Desbloqueado por PA-146a:** no tenía sentido diseñar un certificado que se firma con una clave que la mitad de los despliegues no tiene y nadie sabía que le faltaba |
