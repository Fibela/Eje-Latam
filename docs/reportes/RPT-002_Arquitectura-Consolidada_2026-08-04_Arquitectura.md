...
| PA-135 | **Cuatro de los seis canales están declarados y no cableados.** `obtener-estado-agente`, `obtener-inventario`, `obtener-estado-boveda` y `consultar-sandbox` no tienen manejador en el agente; sólo responden `consultar-alertas` y `obtener-condiciones` | 🟡 Abierto — **observado el 25-ago-2026** en RPT-079 §11.1, la primera conversación real entre los dos procesos. El rechazo con motivo es correcto y está probado; lo que faltaba era **un punto que dijera que faltan**. La paridad con `contrato-ipc.toml` afirma que los seis están declarados a los dos lados, y ninguna prueba afirma que respondan. El hueco estaba **probado como correcto**, que es la forma más cómoda de no verlo |
| PA-136 | **El plano de control espera al de datos.** Una consulta por el socket tarda entre 444 y 983 ms en local, porque el agente atiende al final del ciclo (RPT-034 §4) y cada ciclo espera hasta `PLAZO` = 500 ms por trama | 🟡 Abierto — RPT-079 §11.2, medido en máquina real. No venció el plazo de 5 s de `ESPERA_MAXIMA_MS`, pero el acoplamiento está confirmado: con `--tramas 200` y un segmento con tráfico, el ciclo se alarga y ese margen deja de ser holgado. Era la hipótesis 5 de §7, escrita antes de ejecutar |
| PA-133 | **Un sensor recién instalado sin configuración firmada no arranca: entra en bucle de reinicios.** La unidad ya no pasa `--interfaz` porque lo dicta la firma (RPT-077); sin firma, `resolver` toma la rama `Ausente`, no encuentra interfaz y devuelve error de uso. Con `Restart=always` eso es ruido cada cinco segundos, no una avería visible | 🟡 Abierto — hallazgo al preparar RPT-079. **Es una incoherencia de RPT-077 §5 consigo mismo**: allí se argumentó exactamente eso para el caso «firma rota» —arrancar y declarar en lugar de morir— y no se aplicó al caso «sin firma». Consecuencia: `configuracionSinFirmar`, estrenada el 17-ago, **no es alcanzable en un despliegue real**. Mecanismo sin cablear recién hecho |
| PA-134 | **La frase de paso del emisor se lee hasta el fin de la entrada, no hasta el salto de línea.** `pedir_frase` usa `read_to_string`: pulsar Enter no termina nada y `eje-manifiesto` se cuelga sin decir por qué. Además, cualquier línea posterior pegada en la terminal **se convierte en parte de la frase** | 🟡 Abierto — observado el 24-ago-2026 aprovisionando la VM de PA-78. Costó tres intentos y parecía error del operador. Va con `read_line`, y con una prueba que fije que la frase termina en el salto de línea. Toca también `docs/Comandos.md` §6.4, que no advierte de esto |
