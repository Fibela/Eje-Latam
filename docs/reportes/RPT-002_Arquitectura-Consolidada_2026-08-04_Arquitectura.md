...
| PA-142 | **Los ficheros del renderer son ciegos.** `vis04.js` y `diagnostico.js` no los compila `tsc`, no los cruza `dependency-cruiser` y ninguna prueba los ejecuta. Todo lo que se comprueba de ellos se comprueba **leyendo su texto**, y eso es fragil ante el formateador | 🟡 Abierto — RPT-085 §3. La salida no es un analizador de sintaxis mejor: es escribirlos en TypeScript y emitirlos, para que dejen de ser ciegos y las barreras textuales se puedan retirar |
