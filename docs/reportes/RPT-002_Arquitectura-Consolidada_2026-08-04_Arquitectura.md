...
| PA-138b | **Componer `NodoInventario` y cablear el canal.** Con `inventario()` ya hay donde leer; falta que exista que decir | 🟡 Abierto — RPT-087 §5. **Bloqueado por PA-139**: de los cuatro campos, `clase` cruza dos taxonomias y `postura` no tiene valor para «no se sabe». Y `identificador` seria hoy la misma cadena que `direccionEnlace`, asi que sobra uno de los dos |
| PA-139 | **`Postura` no tiene valor para «no se sabe», y mezcla juicio con medida.** Declara `conforme\|anomalo\|contenido`: los tres afirman algo sobre el mundo. Un equipo visto en el cable sin marcado firmado no es ninguno. Y `contenido` es un **estado operativo**, no una postura: contener a un equipo borra la razón por la que se contuvo | 🟡 Abierto — RPT-081 §5. `Postura::Contenido` es además **inalcanzable**: el agente no contiene nada (RPT-020). El cambio —cuarta postura `indeterminado` y contención a su propio campo— cruza contrato, `eje-ipc`, TypeScript, VIS-04 y la paridad de los dos lados, así que se hará **una vez y con evidencia**, no antes de PA-138 | **Desbloqueado 28-ago-2026 (RPT-087):** PA-138a entrego la evidencia que faltaba —lo que el almacen sabe de verdad de cada equipo— y ademas anadio dos preguntas a la mesa: `clase` puede venir del marcado firmado **o** de `Protocolo::clase_sugerida`, que es una inferencia y no una declaracion; e `identificador` sobra si es la MAC serializada |
