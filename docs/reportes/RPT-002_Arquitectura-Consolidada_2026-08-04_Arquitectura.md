...
| PA-149 | **Qué condiciones son «manipulación» está escrito a mano en dos idiomas y nada las compara.** `hay_manipulacion()` en Rust lleva una lista; la cascada de `componerCabecera` en TypeScript lleva otra. Cuando PA-146b-1 añadió las dos de recuperación al lado de Rust, la de TypeScript se quedó con dos — y el comentario que la acompañaba decía que respetaba la separación de Rust «en lugar de reinventarla», **mientras la reinventaba** | 🟡 Abierto — 10-sep-2026. Corregido el síntoma en RPT-096, no la causa: las dos listas siguen sin nada que las coteje. **Lo encontró una captura de pantalla**, no una prueba: en `eje-prueba`, con la clave de recuperación sustituida y el agente diciendo «trate este equipo como comprometido», la cabecera titulaba sobre el colector que falta. Décimo índice escrito a mano de la serie, y el primero que atraviesa la frontera de lenguaje |
